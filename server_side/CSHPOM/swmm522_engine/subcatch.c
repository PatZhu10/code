...
//=============================================================================
void subcatch_setParams(int i, double x[])
// 
...
//
{
    // Subcatch[i].area = x[0] / UCF(LANDAREA);
    // Subcatch[i].width = x[1] / UCF(LENGTH);
    // Subcatch[i].slope       = x[2] / 100.0;
    // Subcatch[i].fracImperv = MIN(x[3], 100.0) / 100.0;
    // Subcatch[i].subArea[IMPERV0].N = x[4];
    // Subcatch[i].subArea[IMPERV1].N = x[4];
    // Subcatch[i].subArea[PERV].N = x[5];
    // Subcatch[i].subArea[IMPERV0].dStore = 0.0;
    // Subcatch[i].subArea[IMPERV1].dStore = x[6] / UCF(RAINDEPTH);
    // Subcatch[i].subArea[PERV].dStore    = x[7] / UCF(RAINDEPTH);
    // Subcatch[i].subArea[IMPERV0].fArea  = Subcatch[i].fracImperv * x[8] / 100.0;

    Subcatch[i].fracImperv = MIN(x[0], 100.0) / 100.0;
    Subcatch[i].subArea[IMPERV0].N = x[1];
    Subcatch[i].subArea[IMPERV1].N = x[1];
    Subcatch[i].subArea[PERV].N = x[2];
    Subcatch[i].subArea[IMPERV0].dStore = 0.0;
    Subcatch[i].subArea[IMPERV1].dStore = x[3] / UCF(RAINDEPTH);
    Subcatch[i].subArea[PERV].dStore    = x[4] / UCF(RAINDEPTH);
    Subcatch[i].subArea[IMPERV0].fArea  = Subcatch[i].fracImperv * x[5] / 100.0;
