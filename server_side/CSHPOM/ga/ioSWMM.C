                ...
                else
                {
                    ...
                    else if (section == "REAL_NODE_HEAD")
                    {
                        ...
                        else
                        {
                            ...
                            iss >> WLS_nodeHead.name >> WLS_nodeHead.date >> WLS_nodeHead.time >> WLS_nodeHead.value;
...
// Write [SUBCATCHMENTS]
    ...
    for (std::size_t i = 0; i < I_subcatchmentData.size(); i++)
    {
        ...
        oss << subcatchment.Name << "\t" << subcatchment.Raingage << "\t" << subcatchment.Outlet << "\t"
            << subcatchment.Area << "\t" << best_swmmParam.stored_subcatchFracImperN[i] << "\t" << subcatchment.Width << "\t"
...
// Write [SUBAREAS]
    ...
    for (std::size_t i = 0; i < I_subareaData.size(); i++)
    {
        ...
        oss << subarea.Name << "\t" << best_swmmParam.stored_subcatchImperv01N[i] << "\t" << best_swmmParam.stored_subcatchPerv01N[i] << "\t"
            << best_swmmParam.stored_subcatchImperv1DStore[i] << "\t" << best_swmmParam.stored_subcatchPerv1DStore[i] << "\t" << best_swmmParam.stored_subcatchImperv0FArea[i] << "\t"
...
// Write [INFILTRATION]
    ...
    for (std::size_t i = 0; i < I_subcatchmentData.size(); i++)
    {
        ...
        oss << subcatchment.Name << "\t" << best_swmmParam.stored_subcatchF0[i] << '\t' << best_swmmParam.stored_subcatchFMin[i] << "\t"
            << best_swmmParam.stored_subcatchDecay[i] << "\t" << best_swmmParam.stored_subcatchRegen[i] << "\t" << 0 << "\n";
    }
    oss << "\n";
...
// Write [LOSSES]
    ...
    for (std::size_t i = 0; i < I_lossesData.size(); i++)
    {
        ...
        oss << losses.Conduit << "\t" << best_swmmParam.stored_LinkCLossInlet[i] << "\t" << losses.Kexit << "\t"
