...
// Sort the subcatchment parameters that created by genome (step 1)
    for (int col = 0; col < 6; col++)
    {
        if (col == 0)
        {
            for (int row = 0; row < (SubcatchNum); row++)
            {
                sortedRandomSubcatchParam[row][col] = random_swmmParam.stored_subcatchFracImperN[row];
            }
        }
        if (col == 1)
        {
...
// Format:
// 
//     subcatchmentID  F0   FMin    Regen   Decay
//                    p[0]  p[1]    p[2]    p[3]
//-----------------------------------------------------------------------------
...
// Sort the infiltration parameters that created by genome (step 1)
    ...
    for (int col = 0; col < 4; col++)
    {
        ...
        if (col == 2)
        {
            for (int row = 0; row < (SubcatchNum); row++)
            {
                sortedRandomInfilParam[row][col] = random_swmmParam.stored_subcatchRegen[row];
            }
        }
        if (col == 3)
        {
