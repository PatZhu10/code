...
static double getInfilValue(int index, int property);
...
//  Purpose: retrieves the value of an object's property.
{
    ...
    if (property < 600)
        return getInfilValue(property, index);
    return 0;
}
...
//  Purpose: retrieves current value of a subcatchment's property.
{
    ...
    switch (property)
    {
        ...
        case swmm_SUBCATCH_SLOPE:
          return subcatch->slope * 100.0;
        case swmm_SUBCATCH_FRACIMPERV:
          if (subcatch->fracImperv * 100.0 > 100.0)
            return subcatch->fracImperv;
          else 
            return subcatch->fracImperv * 100.0;
        case swmm_SUBCATCH_IMPERV01_N:
          subcatch->subArea[IMPERV0].N = subcatch->subArea[IMPERV1].N;
          return subcatch->subArea[IMPERV0].N;
        case swmm_SUBCATCH_PERV_N:
          return subcatch->subArea[PERV].N;
        case swmm_SUBCATCH_IMPERV0_DSTORE:
          return subcatch->subArea[IMPERV0].dStore;
        case swmm_SUBCATCH_IMPERV1_DSTORE:
          return subcatch->subArea[IMPERV1].dStore * UCF(RAINDEPTH);
        case swmm_SUBCATCH_PERV_DSTORE:
          return subcatch->subArea[PERV].dStore * UCF(RAINDEPTH);
        case swmm_SUBCATCH_IMPERV0_FAREA:
          return (subcatch->subArea[IMPERV0].fArea * 100.0) / subcatch->fracImperv;
        /*new*/
        default:
          return 0;
    }
...
double getInfilValue(int property, int index)
//
//  Input:   property = a subcatchment property code
//           index = the index of a subcatchment
//  Output:  returns current property value
//  Purpose: retrieves current value of a infiltration's property.
{
    THorton *infil;

    if (index < 0 || index >= Nobjects[SUBCATCH])
        return 0;
    infil = &InfilMethod[index].horton;
    switch (property)
    {
        case swmm_INFILTRATION_F0:
          return infil->f0 * UCF(RAINFALL);
        case swmm_INFILTRATION_FMIN:
          return infil->fmin * UCF(RAINFALL);
        case swmm_INFILTRATION_DECAY:
          return infil->decay * 3600;
        case swmm_INFILTRATION_REGEN:
          if (infil->regen == 0)
            return infil->regen = TINY;
          else
            return -log(1.0-0.98) / (infil->regen * SECperDAY);
        case swmm_INFILTRATION_FMAX:
          return infil->Fmax * UCF(RAINDEPTH);
        default:
          return 0;
    }
}
...
//  Purpose: retrieves current value of a link's property.
{
    ...
    switch (property)
    {
        ...
        case swmm_LINK_ROUGHNESS:
          if (link->type == CONDUIT)
              return Conduit[link->subIndex].roughness;
          else
              return 0;
        case swmm_LINK_OFFSET1:
            return link->offset1 * UCF(FLOW);
        case swmm_LINK_OFFSET2:
            return link->offset2 * UCF(FLOW);
        case swmm_LINK_Q0:
            return link->q0 * UCF(FLOW);
        case swmm_LINK_QLIMIT:
            return link->qLimit * UCF(FLOW);
        case swmm_LINK_CLOSSINLET:
            return link->cLossInlet;
        case swmm_LINK_CLOSSOUTLET:
            return link->cLossOutlet;
        case swmm_LINK_CLOSSAVG:
            return link->cLossAvg;
        case swmm_LINK_SEEPLOSSRATE:
          if (link->type == CONDUIT)
              return Conduit[link->subIndex].seepLossRate * UCF(FLOW);
        case swmm_LINK_YWMAX:
            return link->xsect.ywMax * UCF(LENGTH);
        case swmm_PUMP_CURVE:
          if (link->type == PUMP)
            return Pump[link->subIndex].pumpCurve;
          else
            return 0;
        case swmm_PUMP_INITIALSETTING:
          if (link->type == PUMP)
            return Pump[link->subIndex].initSetting;
          else
            return 0;
        case swmm_PUMP_YON:
          if (link->type == PUMP)
            return Pump[link->subIndex].yOn;
          else
            return 0;
        case swmm_PUMP_YOFF:
          if (link->type == PUMP)
            return Pump[link->subIndex].yOff;
          else
            return 0;
