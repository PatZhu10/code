...
void modifyPumpControlRules(int pumpIndex, int nodeIndex, double startDepth, double stopDepth)
{
    struct TRule* rule;
    struct TPremise* premise;
    struct TAction* action;
    int r;

    // --- Check if "StartPump" rule already exists
    for (r = 0; r < RuleCount; r++)
    {
        if (strcmp(Rules[r].ID, "StartPump") == 0)
        {
            // Loop through premises to update start depth if the condition exists
            premise = Rules[r].firstPremise;
            while (premise)
            {
                if (premise->lhsVar.object == r_NODE &&
                    premise->lhsVar.index == nodeIndex &&
                    premise->lhsVar.attribute == r_DEPTH)
                {
                    premise->value = startDepth;  // Update the pump start depth
                }
    ...
    // --- If the rule doesn't exist, creat "StartPump" rule
    r = RuleCount++;
    Rules[r].ID = strdup("StartPump");
    Rules[r].priority = 1.0;  // Assign a priority level

    // --- Creat a condition: If Node depth > startDepth
    premise = (struct TPremise*)malloc(sizeof(struct TPremise));
    premise->type = r_IF;
    premise->lhsVar.object = r_NODE;
    premise->lhsVar.index = nodeIndex;
    premise->lhsVar.attribute = r_DEPTH;
    premise->relation = GT;  // Greater than condition
    premise->value = startDepth;
    premise->next = NULL;
    Rules[r].firstPremise = premise;
    Rules[r].lastPremise = premise;

    // --- Create an action: Turn [ON] the pump
    action = (struct TAction*)malloc(sizeof(struct TAction));
    action->rule = r;
    action->link = pumpIndex;
    action->attribute = r_STATUS;
    action->value = 1.0;  // Set pump status to [ON]
    action->next = NULL;
    Rules[r].thenActions = action;

    // --- Repeat the same process for "StopPump" rule
    r = RuleCount++;
    Rules[r].ID = strdup("StopPump");
    Rules[r].priority = 1.0;

    // --- Create a condition: If Node depth < stopDepth
    premise = (struct TPremise*)malloc(sizeof(struct TPremise));
    premise->type = r_IF;
    premise->lhsVar.object = r_NODE;
    premise->lhsVar.index = nodeIndex;
    premise->lhsVar.attribute = r_DEPTH;
    premise->relation = LT;  // Less than condition
    premise->value = stopDepth;
    premise->next = NULL;
    Rules[r].firstPremise = premise;
    Rules[r].lastPremise = premise;

    // --- Create an action: Turn [OFF] the pump
    action = (struct TAction*)malloc(sizeof(struct TAction));
    action->rule = r;
    action->link = pumpIndex;
    action->attribute = r_STATUS;
    action->value = 0.0;  // Set pump status to [OFF]
    action->next = NULL;
    Rules[r].thenActions = action;
}
//=============================================================================