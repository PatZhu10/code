...
//  Local functions
//-----------------------------------------------------------------------------
extern int    horton_setParams(THorton *infil, double p[]);
...
//
{
    InfilMethod = (TInfil *) calloc(n, sizeof(TInfil));
    if (InfilMethod == NULL) ErrorCode = ERR_MEMORY;
    InfilFactor = 1.0;
    return;
}
...
//     subcatch  p1  p2 ... (infilMethod)
{
    ...
    {
      case HORTON:
      case MOD_HORTON:   status = horton_setParams(&InfilMethod[j].horton, x);
                         break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:
                         status = grnampt_setParams(&InfilMethod[j].grnAmpt, x);
                         break;
      case CURVE_NUMBER: status = curvenum_setParams(&InfilMethod[j].curveNum, x);
                         break;
      default:           status = TRUE;
    }
...
//
{
    ...
    {
      case HORTON:
      case MOD_HORTON:   horton_initState(&InfilMethod[j].horton);   break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:
                         grnampt_initState(&InfilMethod[j].grnAmpt);    break;
      case CURVE_NUMBER: curvenum_initState(&InfilMethod[j].curveNum);   break;
    }
}
...
//
{
    ...
    {
      case HORTON:
      case MOD_HORTON:   horton_getState(&InfilMethod[j].horton, x); break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:
                         grnampt_getState(&InfilMethod[j].grnAmpt, x);   break;
      case CURVE_NUMBER: curvenum_getState(&InfilMethod[j].curveNum, x); break;
    }
}
...
//
{
    ...
    {
      case HORTON:
      case MOD_HORTON:   horton_setState(&InfilMethod[j].horton, x); break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:
                         grnampt_setState(&InfilMethod[j].grnAmpt, x);   break;
      case CURVE_NUMBER: curvenum_setState(&InfilMethod[j].curveNum, x); break;
    }
}
...
//
{
    ...
    {
      case HORTON:
          return horton_getInfil(&InfilMethod[j].horton, tstep, rainfall+runon, depth);

      case MOD_HORTON:
          return modHorton_getInfil(&InfilMethod[j].horton, tstep, rainfall+runon,
                                    depth);

      case GREEN_AMPT:
      case MOD_GREEN_AMPT:
        return grnampt_getInfil(&InfilMethod[j].grnAmpt, tstep, rainfall+runon, depth,
            Subcatch[j].infilModel);

      case CURVE_NUMBER:
        depth += runon * tstep;
        return curvenum_getInfil(&InfilMethod[j].curveNum, tstep, rainfall, depth);
...
//
{
    p[0] = InfilMethod[j].grnAmpt.S * UCF(RAINDEPTH);   // Capillary suction head (ft)
    p[1] = InfilMethod[j].grnAmpt.Ks * UCF(RAINFALL);   // Sat. hyd. conductivity (ft/sec)
    p[2] = InfilMethod[j].grnAmpt.IMDmax;               // Max. init. moisture deficit
}
...
void infil_setModel(int j, int modelType)
{

    // Set the new infiltration model
    Subcatch[j].infilModel = modelType;

    // Reinitialize the corresponding infiltration parameters
    switch (modelType)
    {
      case HORTON:
      case MOD_HORTON:
          horton_initState(&InfilMethod[j].horton);
          break;
      case GREEN_AMPT:
      case MOD_GREEN_AMPT:
          grnampt_initState(&InfilMethod[j].grnAmpt);
          break;
      case CURVE_NUMBER:
          curvenum_initState(&InfilMethod[j].curveNum);
          break;
      default:
          // Handle invalid model type
          printf("Error: Invalid infiltration model type.\n");
          return;
    }
}